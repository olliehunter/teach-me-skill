...
`format` ∈ `single_choice | multi_choice | true_false` for v1. **Authoring rule:** all options should be near-identical in length/word-count so formatting leaks no clue to the answer. The player plays **`audio_correct`** when the learner picks `answer`, and **`audio_incorrect`** for any wrong choice — so `incorrect_feedback` is the real teaching moment: it must state the correct answer *and* briefly say why it's right. All three clips (`audio_intro`, `audio_correct`, `audio_incorrect`) are pre-rendered.
