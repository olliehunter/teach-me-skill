...
def validate_lesson(root: Path, path: Path, require_audio: bool):
    ...
    for b in L.get("beats", []):
        ...
        if b["type"] == "narration":
            ...
            # option length parity: warn if lengths vary a lot (formatting could leak the answer)
            ...
            # separate correct/incorrect feedback so the player can respond to the actual choice
            if not b.get("incorrect_feedback"):
                warn(f"{tag}: no incorrect_feedback — the learner hears nothing useful when wrong")
